...
# 날짜 인덱스로 슬라이싱할 때는, 이전에 정렬이 되어있어야 함! (sort_index())
print('-' * 60)
...
for idx in [edge-1, edge, edge+1]:
    w = f"{wrong[idx]:,.0f}" if pd.notna(wrong[idx]) else "NaN"
    r = f"{right[idx]:,.0f}" if pd.notna(right[idx]) else "NaN"

    print(f" {idx:<8} {df.loc[idx, 'code']:<9} {df.loc[idx, 'close']:>10,} {w:>20} {r:>20}")

    """
        groupby 없이 계산하면 (wrong) 종목별 코드가 달라도 이평선(이동평균선) 재계산없이 이어지는 형태
