...
print(f"1으로 채워서 배열 생성: {np.ones(5)}")  # float 타입으로 생성
print(f"특정 값으로 채워서 배열 새성: {np.full(5, 7)}") # 전달값의 데이터 타입에 따라 형성이 바뀜
print(f"0부터 9까지 정수를 생성하여 배열 생성: {np.arange(10)}")
print(f"0부터 100까지 5등분하여 배열 새성: {np.linspace(0, 100, 5)}")
